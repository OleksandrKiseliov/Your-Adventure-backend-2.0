...
namespace YourAdventure.BusinessLogic.Services
{
    public class InterfaceLanguageGenerator : IInterfaceLanguageGenerator
    {
        ...
        {
            using var connection = new SqlConnection(_config.GetConnectionString("DefaultConnection"));
            await connection.ExecuteAsync("insert into InterfaceLanguage (InterfaceLanguageId, InterfaceLanguage)" +
                " values (@InterfaceLanguageId, @InterfaceLanguageName)", interfaceLanguage);
